...
public class PlaybackTests extends BaseTests {
    ...
    // Helper method to navigate to playlist and start playback
    private void goToPlayablePageAndStartPlayback() {
        driver.get("https://open.spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M");
        ...
        wait.until(driver -> !playbackPage.getPlayPauseState().isEmpty());
    }
    ...
    public void testPlayPauseToggle() {
        ...
        PlaybackPage playbackPage = new PlaybackPage(driver);
        String state = playbackPage.getPlayPauseState();

        Assert.assertEquals(state, "Pause", "Should be in pause state after playing.");
    }
    ...
    public void testShuffleToggle() {
        ...
        PlaybackPage playbackPage = new PlaybackPage(driver);
        boolean initialState = playbackPage.getShuffleState();

        playbackPage.toggleShuffle();

        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
        wait.until(driver -> playbackPage.getShuffleState() != initialState);

        boolean finalState = playbackPage.getShuffleState();
        Assert.assertNotEquals(finalState, initialState, "Shuffle state should toggle.");
    }
    ...
    public void testSkipForward() {
        ...
        PlaybackPage playbackPage = new PlaybackPage(driver);
        String initialTitle = playbackPage.getNowPlayingTitle();

        playbackPage.skipForward();

        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
        wait.until(driver -> !playbackPage.getNowPlayingTitle().equals(initialTitle));

        String newTitle = playbackPage.getNowPlayingTitle();
        Assert.assertNotEquals(newTitle, initialTitle, "Track should have changed after skip.");
    }
    ...
    public void testVolumeControlsVisible() {
        ...
        PlaybackPage playbackPage = new PlaybackPage(driver);
        Assert.assertTrue(playbackPage.isVolumeVisible(), "Volume controls should be visible.");
    }
}